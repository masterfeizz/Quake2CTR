...
void Touch_Init(){
	...
	touchpad_overlay = touchOverlay_bin;
	keyboard_overlay = keyboardOverlay_bin;
...
void Touch_KeyboardToggle()
{
	...
	{
		shift_toggled = 0;
		Key_Event(K_SHIFT,false, Sys_Milliseconds());
	}
...
void Touch_TouchpadUpdate()
{
	...
	{
		if(last_key)
			Key_Event(last_key, false, Sys_Milliseconds());
		if(key)
...
void Touch_KeyboardUpdate()
{
	...
	if(key == K_SHIFT && key != last_key)
	{
		...
		Key_Event(K_SHIFT, shift_toggled, Sys_Milliseconds());
		Touch_DrawOverlay();
	}
